...
if (axios) {
    test("translate using axios if default fetch, and installed", async (t) => {
        const res = await translate("translator", {
            from: "en",
            to: "nl",
...
test("try to translate with an unsupported request function", async (t) => {
    try {
        await translate("something", {
            from: "en",
            to: "es",
            requestFunction: "test",
        });
        t.fail();
    } catch (err) {
        t.is(err.code, 400);
...
test("autoCorrect", async (t) => {
    let res = await translate("I spea Dutch!", {
        from: "en",
        to: "nl",
        autoCorrect: true,
    });
    ...
    if (!res.from.text.autoCorrected) {
        const correctedText = res.from.text.value.replace(
            /\[([a-z]+)\]/gi,
            "$1"
        );
...
test("array input in auto from different languages", async (t) => {
    const res = await translate(["perro", "犬", "كلب"], {
        from: "auto",
        to: "en",
    });
